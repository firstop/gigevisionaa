...
{
    tpacket_req3 req = {0};
    req.tp_block_size = getpagesize() << 2;        // 16384 octets, soit 1 trame en MTU jumbo frame
    req.tp_frame_size = TPACKET_ALIGNMENT << 6;    // 1024 octets, le plus petit packet GVSP étant 576
    req.tp_block_nr = 512;                         //
...
int GvspReceiverPrivate::nicIndexFromAddress(const QHostAddress &address)
{
    ...
    // on cherche l'interface correspondant à l'IP
    int index = -1;
...
void GvspReceiver::run()
{
    ...
    // obteint l'ordonnancement temps réel
    d->setRealtime();
...
void GvspReceiverPrivate::userStack(int socketDescriptor)
{
    ...
    // variables globales à la boucle
    int pollResult = 0;

    const int VLEN = 8;
    // allocation et initialisation réelle des tampons
    uint8_t bufs[VLEN][GVSP_BUFFER_SIZE] = {{0}};
    // tableau de structures à passer à recvmmsg
    mmsghdr msgs[VLEN] = {0};
    // pour le mappage des tampons
    iovec iovecs[VLEN] = {0};
    // mappage de la mémoire réelle
    for (int i = 0; i < VLEN; i++) {
        iovecs[i].iov_base         = bufs[i];           // l'adresse du nième tampon
        iovecs[i].iov_len          = GVSP_BUFFER_SIZE;  // sa taille
        msgs[i].msg_hdr.msg_iov    = &iovecs[i];        // l'adresse du iovec correspondant
        msgs[i].msg_hdr.msg_iovlen = 1;                 // 1 seul élément dans le vecteur
    }
