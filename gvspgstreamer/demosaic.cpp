...
Demosaic::Demosaic()
    : d(new DemosaicPrivate)
{}

Demosaic::~Demosaic()
{}

bool Demosaic::needDemosaic(quint32 format)
{
    return isBayer(format);
}

void Demosaic::allocate(GvspImage &image)
{
    // alloue un tampon RGB pour le demosaic
    if (isBayer(image.geometry.pixelFormat)) {
        RGB_Buffer rgb;
        ...
        case GVSP_PIX_BAYGB8:
        case GVSP_PIX_BAYBG8:
            rgb.initType(image.geometry.width, image.geometry.height, NULL);
            break;
        case GVSP_PIX_BAYGR12:
        case GVSP_PIX_BAYRG12:
        case GVSP_PIX_BAYGB12:
        case GVSP_PIX_BAYBG12:
            rgb.initType(image.geometry.width, image.geometry.height, U122U8());
            break;
        case GVSP_PIX_BAYGR12_PACKED:
        case GVSP_PIX_BAYRG12_PACKED:
        case GVSP_PIX_BAYGB12_PACKED:
        case GVSP_PIX_BAYBG12_PACKED:
            rgb.initType(image.geometry.width, image.geometry.height, U12PACKED2U8());
            break;
        ...
        d->map.insert(image.timestamp, rgb);
    }
...
GstBuffer *Demosaic::transform(GvspImage &image)
{
    RGB_Buffer rgb = d->map.take(image.timestamp);

    // transformation
    if (rgb.needTransform()) {
        // alloue un tampon temporaire pour la conversion vers U8
        uchar tmp[rgb.pixelsCount];
        rgb.buffer2U8(reinterpret_cast<uchar *>(image.datas.p), tmp);
        cv::cvtColor(cv::Mat(rgb.height, rgb.width, CV_8U, tmp),
                     cv::Mat(rgb.height, rgb.width, CV_8UC3, rgb.data),
                     CV_BayerGB2RGB);
    }
    else {
        cv::cvtColor(cv::Mat(rgb.height, rgb.width, CV_8U, image.datas.p),
                     cv::Mat(rgb.height, rgb.width, CV_8UC3, rgb.data),
                     CV_BayerGB2RGB);
    }
...
GstCaps *Demosaic::capsForTransform(const GvspImage &image) const
{
    RGB_Buffer rgb = d->map[image.timestamp];
